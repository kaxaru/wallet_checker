...
from utils import generate_account, generate_api_key
from utils.getFile import read_file
from urllib.parse import urlencode
from pyuseragents import random as random_useragent
from utils.debankResult import format_result
from layer.debankType import TokenBalancesResultData, PoolBalancesResultData, NftBalancesResultData, RequestParamsStruct, DebankConfig, ConfigStruct, RequestSessionStruct
from loguru import logger
from debank2.utils.generate_api_key import generate_api_key
...
def do_request(account_address, base_url, method, path, params, payload, proxy, sessionId = None):
    ...
    if method.upper() == "POST":
        ...
        try:
            ...
            nonce, signature, timestamp = debank_params.dict().values()
        except Exception as e:
    ...
    if not sessionId == None:
        try:
            ...
            acc = RequestSessionStruct(**acc)

        except Exception as e:
            print(f"{account_address} | Error decoding account headers: {e}")

        account_header = acc
        account_header  = account_header.json()

    api_key = api_key.__str__()
    headers = {
        'accept': '*/*',
        ...
        while True:
            try:
                ...
                for token in response_data['data']:
                    ...
                    tokens_result_data.append(_data)
            ...
            for current_pool in response_data['data']:
                for item in current_pool['portfolio_item_list']:
                    for token in item['asset_token_list']:
                        ...
                        _pool = PoolBalancesResultData(**{
                            'name': token['name'],
                            'amount': token['amount'],
                            'balance_usd': token_in_usd,
                        })

                        result[current_pool['chain']][current_pool['name']].append(_pool)
            break
        except Exception as e:
            print(f"{account_address} | Error: {e}")
            continue
...
def parse_debank_account(account_data):
    try:
        ...
        if 'wallet' in account_data[0]:
            ...
            account_address = account_data[0].strip()
